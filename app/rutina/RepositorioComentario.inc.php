...
class RepositorioComentario {
    ...
    public static function insertar_comentario($conexion, $comentario) {
        ...
        if (isset($conexion)) {
            try {
                $sql = "INSERT INTO comentarios(autor_id, rutina_id, titulo, texto, fecha) VALUES(:autor_id, :rutina_id, :titulo, :texto, NOW())";
                
                $sentencia = $conexion -> prepare($sql);
                
                $sentencia -> bindParam(':autor_id', $comentario -> obtener_autor_id(), PDO::PARAM_STR);
                $sentencia -> bindParam(':rutina_id', $comentario -> obtener_rutina_id(), PDO::PARAM_STR);
    ...
    public static function obtener_comentarios($conexion, $rutina_id) {
    	$comentarios = array();
    	...
    	if (isset($conexion)) {
    		try {
    			...
    			$sql = "SELECT * FROM comentarios WHERE rutina_id = :rutina_id";
    			
    			$sentencia = $conexion -> prepare($sql);
    			$sentencia -> bindParam(':rutina_id', $rutina_id, PDO::PARAM_STR);
    			$sentencia -> execute();
    			...
    			if (count($resultado)) {
    				foreach ($resultado as $fila) {
    					$comentarios[] = new Comentario($fila['id'], $fila['autor_id'], $fila['rutina_id'], $fila['titulo'], $fila['texto'],
    					$fila['fecha']);
    				}
    			}
    		} catch (PDOException $ex) {
