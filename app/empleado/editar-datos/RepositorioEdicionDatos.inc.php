...
class RepositorioEdicionDatos{
	...
	public static function generar_peticion($conexion, $id_empleado, $url_secreta){
		...
		if (isset($conexion)) {
			try{
				$sql = "INSERT INTO editar_datos_empleado(empleado_id, url_secreta, fecha) VALUES (:empleado_id, :url_secreta, NOW())";
	...
	public static function url_secreta_existe($conexion, $url_secreta){
		...
		if (isset($conexion)) {
			try{
				$sql = "SELECT * FROM editar_datos_empleado WHERE url_secreta = :url_secreta";
