...
if(isset($_POST['guardar_imagen']) && !empty($_FILES['archivo_subido']['tmp_name'])) {
	$directorio = DIRECTORIO_RAIZ."/usuarios/fotos-perfil/";
	...
	if ($tipo_imagen != "jpg" && $tipo_imagen != "png" && $tipo_imagen != "jpeg" && $tipo_imagen != "gif") {
		...
		DIRECTORIO_RAIZ."/usuarios/fotos-perfil/".$usuario->obtener_cedula())) {
			?>
...
{
@unlink(DIRECTORIO_RAIZ."/usuarios/fotos-perfil/".$usuario->obtener_cedula());
header("Location: #");
}
?>